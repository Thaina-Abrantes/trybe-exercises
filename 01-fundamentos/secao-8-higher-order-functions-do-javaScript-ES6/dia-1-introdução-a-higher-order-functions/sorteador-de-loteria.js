...
function check (apostedNumber, sortedNumber) {
    if (apostedNumber === sortedNumber) {
       return 'Tente novamente';
    }
    return 'Parabéns você ganhou';
}

const sortResult = (number, check) => {
    const sortedNumber = Math.floor(Math.random() * (5 - 1) + 1);
    return check(number, sortedNumber);
}

console.log(sortResult(2, check));
